...
void test_planner::TestPlanner::initialize(std::string name, tf::TransformListener *tf,
...
<<<<<<< Updated upstream
  ...
  // std::cin>>i;
  odom_subscriber_ =
      nh_.subscribe("/odom", 1, &test_planner::TestPlanner::odomCallback, this);  // subscribe to odom messages
=======

  odom_subscriber_ = nh_.subscribe("/odom", 1, &test_planner::TestPlanner::odomCallback, this);
  got_odom_ = false;
>>>>>>> Stashed changes
...
{
  if (!got_odom_)
    return;  // give up if don't have any odom values
...
<<<<<<< Updated upstream
  ...
  {  // if done with path, then halt--but keep servoing on orientation
    compute_omega_rotate_to_phi(heading_err_, current_omega_cmd_);
    cmd_vel.linear.x = 0;
    cmd_vel.angular.z = current_omega_cmd_;
  }
...
void test_planner::TestPlanner::compute_omega_rotate_to_phi(double heading_err, double &omega_cmd)
{
  ...
  if (fabs(omega_cmd) > max_reorientation_omega_)
  {  // make this rotation slower
    omega_cmd = max_reorientation_omega_ * sgn(omega_cmd);
=======
    ds12 = sqrt(dx12 * dx12 + dy12 * dy12);
    speed = ds12 / controller_dt_;
    phi12 = atan2(dy12, dx12);
    ...
    if (ipose_ + 1 < nposes_)
    {
      ...
      dx23 = x3 - x2;
      dy23 = y3 - y2;
      ds23 = sqrt(dx23 * dx23 + dy23 * dy23);
      phi23 = atan2(dy23, dx23);
...
bool test_planner::TestPlanner::update_path_subgoal()
{
  ...
  ipose_++;
  if (ipose_ >= nposes_ - 1)
    return true;                // out of poses, so done with path
...
{
  double lookahead_dist = path_lookahead_dist(des_progress_to_subgoal_, ipose_, des_lin_brake_dist_, g_plan_);
  ...
  if (lookahead_dist < des_lin_brake_dist_)
  {
    ...
    if (des_state_vel_ > nom_travel_speed_)
      des_state_vel_ = nom_travel_speed_;
...
double test_planner::TestPlanner::path_lookahead_dist(double dist_progress_to_node_i, int node_i,
...
double test_planner::TestPlanner::dist_btwn_poses(geometry_msgs::PoseStamped pose1, geometry_msgs::PoseStamped pose2)
{
