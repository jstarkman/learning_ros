...
public:
  ...
  /**provide an array of 3-D points (in columns), and this function will use and eigen-vector approach to find the
  * best-fit plane
  ...
  */
  void fit_points_to_plane(Eigen::MatrixXf points_array, Eigen::Vector3f &plane_normal, double &plane_dist);
  ...
  void fit_points_to_plane(pcl::PointCloud<pcl::PointXYZ>::Ptr input_cloud_ptr, Eigen::Vector3f &plane_normal,
                           double &plane_dist);
  ...
  void copy_cloud_xyzrgb_indices(PointCloud<pcl::PointXYZRGB>::Ptr inputCloud, vector<int> &indices,
                                 PointCloud<pcl::PointXYZRGB>::Ptr outputCloud);

  void get_indices(vector<int> &indices)
  {
    indices = indices_;
  };
  ...
  void filter_cloud_z(PointCloud<pcl::PointXYZ>::Ptr inputCloud, double z_nom, double z_eps, double radius,
  ...
  double find_table_height(double x_min, double x_max, double y_min, double y_max, double z_min, double z_max,
                           double dz_tol);
  ...
  bool find_plane_fit(double x_min, double x_max, double y_min, double y_max, double z_min, double z_max, double dz_tol,
                      Eigen::Vector3f &plane_normal, double &plane_dist, Eigen::Vector3f &major_axis,
  ...
  void find_indices_color_match(vector<int> &input_indices, Eigen::Vector3d normalized_avg_color,
...
private:
  ...
  void initializeSubscribers();  // we will define some helper methods to encapsulate the gory details of initializing
