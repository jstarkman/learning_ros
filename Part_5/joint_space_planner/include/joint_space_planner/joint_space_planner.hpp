...
private:
  ...
  int nlayers_;  // number of "layers" in path_options; e.g., a "layer" may corresponding to a nominal tool pose,for
                 // which there are many IK options
  Eigen::VectorXd diff_;
  Eigen::VectorXd diff_sqd_;
  double min_total_trip_cost_;
  void constructor_helper_();
  void augment_all_min_costs_(std::vector<std::vector<double> > &humerus_sensitivities);

public:
  JointSpacePlanner(std::vector<std::vector<Eigen::VectorXd> > &path_options,
                    Eigen::VectorXd weights);  // option to provide weights
  ...
  double score_move(Eigen::VectorXd pose1, Eigen::VectorXd pose2);  // compute incremental cost to go from pose1 to
                                                                    // pose2, weighted, possibly squared
  bool find_best_moves_single_layer(int target_layer_index);        // compute optimal choices for transitions to layer
                                                                    // target_layer_index
  bool find_best_moves_single_layer(std::vector<std::vector<Eigen::VectorXd> > &path_options, int target_layer_index);
  bool compute_all_min_costs();
  bool compute_all_min_costs(std::vector<std::vector<Eigen::VectorXd> > &path_options);
  bool find_best_move(int ipose, int jlayer);  // for layer "j", choose pose-option "i" and find the index of the lowest
                                               // cost-to-go to advance to next layer
  // here's the main function: given the pose options at each "layer" (from constructor), find the optimal joint-space
  ...
  bool compute_optimal_path(std::vector<std::vector<Eigen::VectorXd> > &path_options);
