...
// const double r_goal_max = 0.745; // forbid reach > this, from shoulder to wrist; forces bent elbows and avoids
// singularities
...
public:
  ...
  Eigen::Affine3d fwd_kin_tool_wrt_base_solve(const Vectorq7x1 &q_vec);  // given vector of q angles, compute fwd kin of
                                                                         // tool w/rt right-arm mount
  ...
  Eigen::Matrix4d get_frame2()
  {
    return A_mat_products_[2];
  };
  Eigen::Matrix4d get_frame3()
  {
    return A_mat_products_[3];
  };
  Eigen::Matrix4d get_frame4()
  {
    return A_mat_products_[4];
  };
  Eigen::Matrix4d get_frame5()
  {
    return A_mat_products_[5];
  };
  Eigen::Matrix4d get_frame6()
  {
...
public:
  ...
  bool solve_for_humerus_ang(Eigen::Vector3d w_wrt_0, double q_yaw, double q_elbow,
                             std::vector<double> &q_humerus_solns);

  bool fit_q_to_range(double q_min, double q_max, double &q);

  bool solve_for_shoulder_pitch_ang(Eigen::Vector3d w_wrt_0, double q_yaw, double q_humerus, double q_elbow,
  ...
  int ik_solve_given_qs0(Eigen::Affine3d const &desired_flange_pose_wrt_base, double q_yaw,
