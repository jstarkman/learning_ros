...
{
public:
  Baxter_traj_streamer(ros::NodeHandle *nodehandle);  //"main" will need to instantiate a ROS nodehandle, then pass it
                                                      //to the constructor
  ...
  Eigen::VectorXd get_q_vec_right_arm_Xd()
  {
    return q_vec_right_arm_Xd_;
  };
  Eigen::VectorXd get_q_vec_left_arm_Xd()
  {
    return q_vec_left_arm_Xd_;
  };
...
private:
  ...
  double val_from_subscriber_;  // example member variable: better than using globals; convenient way to pass data from
                                // a subscriber to other member functions
