...
const double DH_q_offsets[7] = { DH_q_offset1, DH_q_offset2, DH_q_offset3, DH_q_offset4,
                                 DH_q_offset5, DH_q_offset6, DH_q_offset7 };
...
const double r_goal_max =
    0.745;                 // forbid reach > this, from shoulder to wrist; forces bent elbows and avoids singularities
...
public:
  ...
  Eigen::Affine3d fwd_kin_flange_wrt_r_arm_mount_solve(const Vectorq7x1& q_vec);  // given vector of q angles, compute
                                                                                  // fwd kin
  Eigen::Affine3d fwd_kin_flange_wrt_r_arm_mount_solve_approx(const Vectorq7x1& q_vec);  // version w/ spherical-wrist
                                                                                         // approx
  ...
  Eigen::Affine3d fwd_kin_tool_wrt_r_arm_mount_solve(const Vectorq7x1& q_vec);  // given vector of q angles, compute fwd
                                                                                // kin of tool w/rt right-arm mount
  ...
  Eigen::Affine3d fwd_kin_tool_wrt_r_arm_mount_solve_approx(const Vectorq7x1& q_vec);  // version w/ spherical-wrist
                                                                                       // approx
  // option to provide tool-transform to be used
  Eigen::Affine3d fwd_kin_tool_wrt_r_arm_mount_solve_approx(
      const Vectorq7x1& q_vec, Eigen::Affine3d A_tool_wrt_flange);  // version w/ spherical-wrist approx
  ...
  Eigen::Affine3d fwd_kin_tool_wrt_torso_solve(const Vectorq7x1& q_vec,
                                               Eigen::Affine3d A_tool_wrt_flange);  // rtns pose w/rt torso frame (base
                                                                                    // frame)
  ...
  Eigen::Matrix3d get_wrist_Jacobian_3x3(double q_s1, double q_humerus, double q_elbow,
                                         double q_forearm);              // 3x3 J for wrist point coords
  Eigen::Vector3d get_wrist_coords_wrt_frame1(const Vectorq7x1& q_vec);  // fwd kin from frame 1 to wrist pt

  // this fnc casts an affine matrix w/rt torso frame into an affine matrix w/rt right-arm mount frame, so can use fncs
  // above
  ...
  Eigen::Matrix4d A_mats_[7],
      A_mat_products_[7];  //, A_tool; // note: tool A must also handle diff DH vs URDF frame-7 xform
...
public:
  ...
  Eigen::Affine3d get_flange_frame_from_tool_frame(Eigen::Affine3d tool_frame)
  {
    return tool_frame * A_tool_wrt_flange_inv_;
  }
  ...
  int ik_solve_approx_wrt_torso(Eigen::Affine3d const desired_tool_pose_wrt_torso, Eigen::Affine3d A_tool_wrt_flange,
                                std::vector<Vectorq7x1>& q_solns);

  int ik_wristpt_solve_approx_wrt_torso(Eigen::Affine3d const& desired_flange_pose_wrt_torso,
                                        std::vector<Vectorq7x1>& q_solns);

  int ik_solve_approx_elbow_orbit_from_flange_pose_wrt_torso(Eigen::Affine3d const& desired_flange_pose_wrt_torso,
                                                             std::vector<std::vector<Eigen::VectorXd> >& path_options);
  int ik_solve_approx_elbow_orbit_plus_qdot_s0_from_flange_pose_wrt_torso(
  ...
  int ik_solve_approx_wrt_torso_given_qs0(Eigen::Affine3d const& desired_flange_pose_wrt_torso, double q_s0,
                                          std::vector<Vectorq7x1>& q_solns);
  int ik_wrist_solve_approx(Eigen::Affine3d const& desired_flange_pose,
                            std::vector<Vectorq7x1>& q_solns_123);  // given desired hand pose, find all viable IK solns

  // function to find precise values of joint angles q1, q2, q3 to match desired wrist position, implied by
  // desired_flange_pose
  // provide q123_approx; this function will take q_s0 and q_forearm as specified, and q_s1, q_humerus and q_elbow as
  // approximated,
  ...
  double precise_soln_q123(Eigen::Affine3d const& desired_flange_pose, Vectorq7x1 q123_approx,
  ...
  bool improve_7dof_soln(Eigen::Affine3d const& desired_flange_pose_wrt_arm_mount, Vectorq7x1 q_in,
                         Vectorq7x1& q_7dof_precise);
  bool improve_7dof_soln_wrt_torso(Eigen::Affine3d const& desired_flange_pose_wrt_torso, Vectorq7x1 q_in,
                                   Vectorq7x1& q_7dof_precise);
...
private:
  ...
  // Eigen::Matrix4d A_mats[7], A_mat_products[7], A_tool; // note: tool A must also handle diff DH vs URDF frame-7
  // xform
  double L_humerus_;
  double L_forearm_;
  // double phi_elbow_;
