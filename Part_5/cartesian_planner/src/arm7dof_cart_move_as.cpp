...
void set_jnt_names()
{
  g_jnt_names.push_back("joint0");
  g_jnt_names.push_back("joint1");
  g_jnt_names.push_back("joint2");
  g_jnt_names.push_back("joint3");
  g_jnt_names.push_back("joint4");
  g_jnt_names.push_back("joint5");
  g_jnt_names.push_back("joint6");
}
...
private:
  ...
  actionlib::SimpleActionClient<arm7dof_traj_as::trajAction> action_client_;

  cartesian_planner::cart_moveGoal cart_goal_;
  cartesian_planner::cart_moveResult cart_result_;

  arm7dof_traj_as::trajGoal js_goal_;
  arm7dof_traj_as::trajResult js_result_;

  void armDoneCb_(const actionlib::SimpleClientGoalState& state, const arm7dof_traj_as::trajResultConstPtr& result);
...
public:
  ...
  bool plan_path_current_to_goal_flange_pose();
  ...
  , action_client_("trajActionServer", true)
{
  ...
  while (tferr)
  {
    ...
    try
    {
      tfListener_->lookupTransform("generic_gripper_frame", "link8", ros::Time(0),
    ...
    catch (tf::TransformException& exception)
    {
      ...
      ntries++;
      if (ntries > 5)
        ROS_WARN("did you launch cartesian_planner/arm7dof_static_transforms.launch?");
    }
  }
  ...
  while (tferr)
  {
    ...
    try
    {
      tfListener_->lookupTransform("system_ref_frame", "link1", ros::Time(0), base_link_wrt_system_ref_frame_stf_);
    }
    catch (tf::TransformException& exception)
    {
      ROS_WARN("%s; retrying...", exception.what());
...
{
  q_vec_ = g_q_vec_arm_Xd;
  affine_tool_wrt_base_ = arm7dof_fwd_solver.fwd_kin_flange_wrt_base_solve(q_vec_);
  ...
  current_gripper_stamped_pose_.header.frame_id = "link1";
}

void ArmMotionInterface::compute_flange_stamped_pose(void)
{
  q_vec_ = g_q_vec_arm_Xd;
  affine_flange_wrt_base_ = arm7dof_fwd_solver.fwd_kin_flange_wrt_base_solve(q_vec_);
  ...
  current_flange_stamped_pose_.header.frame_id = "link1";
}
